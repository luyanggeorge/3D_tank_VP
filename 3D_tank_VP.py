...
if case=='TC1': # save the energy file separately
    ...
    x_coor = np.linspace(0+0.1, Lx-0.1, 20) # depends on the coarse mesh
    y_coor = np.linspace(0+0.1, Ly-0.1, 4)
...
while t<Tend: 
    ...
    if t_save-smallfac < t: 
        ...
        if case=='TC1':
            ...
            field_data = np.array(h_n0.at(points))
            # Save the array to a binary file (.npy)
            op_file = os.path.join(save_path, tt+'.npy')
            with open(op_file,'wb') as f:
                np.save(f, field_data)
