...
if input_data=="measurements":#------------- Interpolate measurements -------------#
    ...
    WM_expr(WM, x, t, t_wm, wm_data)
    dWM_dt_expr(dWM_dt, x, t, t_vel, vel_data)
    dWM_dy_expr(dWM_dy)
else:
    # yl update
...
""" *********************************************************************************
    *                                   Time loop                                   *
    ********************************************************************************* """
t_save = t+dt_save # do not save the initial condition
...
while t<=Tend+smallfac: #  while t<Tend-dt: Onno 15-04-2020
    ...
    if t_save-smallfac < t: # Onno 13-04-2020
        ...
        energy_data = np.append(energy_data,np.array([[t,tot_energy]]),axis=0)
        ...
        if case=='TC4':
            probe_data = np.append(probe_data,\
                np.array([[t,*h_n0.at(10, 20, 40, 49.5, 50, 54),WM.at(0),dWM_dt.at(0)]]),axis=0)
    ...
    if input_data=="created":
        ...
        WM_expr(WM_n1,x,t,t_wm,wm_data)
        dWM_dt_expr(dWM_n1_dt,x,t,t_vel,vel_data)
...
with open(energy_file,'w') as e_f:
    np.savetxt(e_f, energy_data, fmt='%10.4f '+' %.18e ')
