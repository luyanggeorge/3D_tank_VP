...
#   If needed some changes should be made as the layers used to be distributed evenly.
...
if input_data=="measurements":#------------- Interpolate measurements -------------#
    # load the data and construct the 1D linear interpolator
    t_wm, wm_data, t_vel, vel_data = load_wavemaker(dt)
    PETSc.Sys.Print('-> Wavemaker data loaded')
    wm_inter  = interp1d(t_wm, wm_data)
    vel_inter = interp1d(t_vel,vel_data)
    
    WM_expr(WM, x, t, wm_inter)
    dWM_dt_expr(dWM_dt, x, t, vel_inter)
    dWM_dy_expr(dWM_dy)
else:
    # yl update
    WM_expr(WM,x,t,t_stop)                  # \tilde{R}(x,y;t)
    dWM_dt_expr(dWM_dt,x,t,t_stop)          # d\tilde{R}/dt  
    dWM_dy_expr(dWM_dy,x,t,t_stop)          # d\tilde{R}/dy
...
while t<=Tend+smallfac: #  while t<Tend-dt: Onno 15-04-2020
    ...
    if t_save-smallfac < t: # Onno 13-04-2020
        ...
        if case=='TC1':
            ...
            probe_data = np.array([[t,*h_n0.at(10, 20, 40, 49.5, 50, 54),WM.at(0),dWM_dt.at(0)]])
            if hor_mesh.comm.rank==0:
    ...
    if input_data=="created":
        if scheme=="SV":
            # updated by yl
            WM_expr(WM_half, x, t_half, t_stop)                            # update R(x,y;t)
            dWM_dt_expr(dWM_half_dt, x, t_half, t_stop)                    # update dR/dt
            dWM_dy_expr(dWM_half_dy, x, t_half, t_stop)                    # update dR/dy
            # yl update: XUVW
        ...
        # updated by yl
        WM_expr(WM_n1, x, t, t_stop)                            # update R(x,y;t)
        dWM_dt_expr(dWM_n1_dt, x, t, t_stop)                    # update dR/dt
        dWM_dy_expr(dWM_n1_dy, x, t, t_stop)                    # update dR/dy
        # yl update: XUVW
        ...
        if scheme=="SV":
            ...
            dWM_dt_expr(dWM_half_dt,x,t_half,vel_inter)                    
            # yl update: XUVW
            Ww_half.interpolate(Lw-WM_half)
            VoW_half.interpolate(Vv_half/Ww_half)
            IoW_half.interpolate(1/Ww_half)
        ...
        dWM_dt_expr(dWM_n1_dt,x,t,vel_inter)
        dWM_dy_expr(dWM_n1_dy)
        # yl update: XUVW
        Ww_n1.interpolate(Lw-WM_n1)
