...
if hatphi_one:
    ...
    if dim =='3D':
        ...
        hat_phi_z.interpolate(hat_phi_expr.dx(2))
    elif dim=='2D':
        hat_phi_expr = product( (x[1]-z_k.item(i))/(H0-z_k.item(i)) for i in range(n_z) )
        hat_phi.interpolate(hat_phi_expr)
        hat_phi_z.interpolate(hat_phi_expr.dx(1))
...
if dim=='2D':
    ...
    '''
    VP_mp= (H0*( psi_mp*W_half*(h_n1-h_n0)/dt - h_mp*(psi_n1*W_n1-psi_n0*W_n0)/dt + psi_mp*X*dWM_half_dt*h_mp.dx(0) ))*ds_t \
        -( (Lw*Lw*h_mp/(2*W_half)) * ( hat_phi*psi_mp.dx(0)+varphi_mp.dx(0) - (x[1]*h_mp.dx(0)/h_mp)*(psi_mp*hat_phi_z+varphi_mp.dx(1)) )**2 \
         + (H0*H0*W_half/(2*h_mp)) * ( psi_mp*hat_phi_z+varphi_mp.dx(1) )**2 \
          + W_half*g*h_mp*( (x[1]*h_mp/H0) - H0 ) )*dx \
        - ( Lw*dWM_half_dt*h_mp*(psi_mp*hat_phi+varphi_mp) )*ds_v(1)
elif dim=='3D':
    VP_mp= (H0*( psi_mp*W_half*(h_n1-h_n0)/dt - h_mp*(psi_n1*W_n1-psi_n0*W_n0)/dt + psi_mp*X*dWM_half_dt*h_mp.dx(0) ))*ds_t \
...
while t<=Tend+smallfac: 
    ...
    if t_save-smallfac < t: 
        ...
        if dim=='2D':
            ...
            kin_energy=assemble( ( (Lw*Lw*h_mp/(2*W_half)) * ( hat_phi*psi_mp.dx(0)+varphi_mp.dx(0) - (x[1]*h_mp.dx(0)/h_mp)*(psi_mp*hat_phi_z+varphi_mp.dx(1)) )**2 \
                                  +(H0*H0*W_half/(2*h_mp)) * ( psi_mp*hat_phi_z+varphi_mp.dx(1) )**2 )*dx )
            #pot_energy=assemble( (W_half*g*h_mp*( (x[1]*h_mp/H0) - H ))*dx )
            pot_energy=assemble( (W_half*g*h_mp*( (x[1]*h_mp/H0) - H0 ))*dx )
        elif dim=='3D':
        ...
        energy_data_row = np.array([[t,kin_energy,pot_energy,tot_energy]])
        PETSc.Sys.Print(energy_data_row)
        energy_data = np.append(energy_data, energy_data_row, axis=0)
        ...
        if case=='TC4':
            probe_data_row = np.array([[t,*h_n0.at([10,0],[20,0],[40,0],[49.5,0],[50,0],[54,0]),WM_n1.at([0,0]),dWM_half_dt.at([0,0])]])
            PETSc.Sys.Print(probe_data_row)
            probe_data = np.append(probe_data, probe_data_row, axis=0)
