...
def test_case():
    ...
    save_path = 'data_VP/'+scheme+'/TC1_2D_series2(1ele,phiGLL)/test1/'
...
def domain(bottom):
    ...
    res_x = 2*pi/200                                            # x-resolution
    res_y = 1.0                                            # y-resolution
    n_z = 10                                         # Order of the expansion
...
def set_time(Lx, res_x, Ly, res_y, H0, n_z, Tw):
    ...
    dz = H0/n_z
    dx = Lx/Nx
    dt = ((Lx/3200)/(2*pi))*32 #(min(dx,dz)/pi)       # time step
    T0 = 0.0                     # Initial time
    t = T0                  # Temporal variable 
    dt_save = dt*2          # saving time step
